/* global fcunited_color_schemes, fcunited_dependencies, Color */
...
( function( api ) {
	...
	for (var i in fcunited_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'fcunited-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'fcunited-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'fcunited-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + fcunited_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( fcunited_customizer_vars['msg_reset_confirm'] )) {
			...
			if (fcunited_customizer_vars && fcunited_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( fcunited_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', fcunited_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', fcunited_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					fcunited_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							fcunited_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			fcunited_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			fcunited_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof fcunited_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-fcunited-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-fcunited-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in fcunited_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-fcunited-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-fcunited-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-fcunited-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'fcunited-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in fcunited_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (fcunited_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = fcunited_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = fcunited_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function fcunited_customizer_get_field_value(fld) {
	...
	function fcunited_customizer_check_dependencies(cont) {
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = fcunited_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'fcunited_options_no_use' );
					} else {
						ctrl.hide().addClass( 'fcunited_options_no_use' );
					}
				}
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = fcunited_customizer_get_field_value( fld );
						var num = 0;
						for (i in fcunited_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function fcunited_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( fcunited_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					fcunited_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in fcunited_theme_fonts) {
					for (rule in fcunited_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							fcunited_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				fcunited_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function fcunited_customizer_actions(e) {
	...
	function fcunited_customizer_update_theme_vars(opt, value) {
		fcunited_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function fcunited_customizer_update_theme_fonts(opt, rule, value) {
		fcunited_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function fcunited_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= fcunited_customizer_vars['max_load_fonts']; i++) {
	...
	function fcunited_customizer_update_css() {
		...
		var vars = fcunited_customizer_add_theme_vars( fcunited_theme_vars );

		// Make styles and add its to the css
		css += fcunited_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = fcunited_customizer_add_theme_fonts( fcunited_theme_fonts );

		// Make styles and add its to the css
		css += fcunited_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in fcunited_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( fcunited_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in fcunited_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in fcunited_color_schemes[scheme].colors) {
				colors[i] = fcunited_color_schemes[scheme].colors[i];
			}
			...
			var tmpl = jQuery( '#tmpl-fcunited-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function fcunited_customizer_add_theme_colors(colors) {
		if (fcunited_additional_colors) {
			var clr = '', v = '';
			for (var k in fcunited_additional_colors) {
				v   = fcunited_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + fcunited_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + fcunited_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + fcunited_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function fcunited_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = fcunited_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']              = fcunited_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = fcunited_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['sidebar']           = fcunited_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']               = fcunited_customizer_prepare_css_value( vars['gap'] );
				rez['content']           = fcunited_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap']       = fcunited_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['gap_prc']           = vars['gap'] / vars['page'];
				rez['page_1_1']          = fcunited_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = fcunited_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = fcunited_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = fcunited_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = fcunited_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = fcunited_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins              = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = fcunited_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = fcunited_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function fcunited_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function fcunited_customizer_prepare_html_value(val) {
