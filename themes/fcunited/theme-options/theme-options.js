		...
		function fcunited_options_check_dependencies(cont) {
			cont.find( '.fcunited_options_item_field' ).each(
				function() {
					...
					var depend = false;
					for (var fld in fcunited_dependencies) {
						if (fld == id) {
					...
					if (depend) {
						...
						for (var i in depend) {
							...
							if (name.charAt( 0 ) == '#' || name.charAt( 0 ) == '.') {
								fld = jQuery( name );
								if (fld.length > 0 && ! fld.hasClass( 'fcunited_inited' )) {
									fld.addClass( 'fcunited_inited' ).on(
										'change', function () {
											jQuery( '.fcunited_options .fcunited_options_section' ).each(
												function () {
													fcunited_options_check_dependencies( jQuery( this ) );
						...
						if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
							ctrl.parents( '.fcunited_options_item' ).show().removeClass( 'fcunited_options_no_use' );
						} else {
							ctrl.parents( '.fcunited_options_item' ).hide().addClass( 'fcunited_options_no_use' );
						}
					}
