<div class="front_page_section front_page_section_features<?php
	$fcunited_scheme = fcunited_get_theme_option( 'front_page_features_scheme' );
	if ( ! fcunited_is_inherit( $fcunited_scheme ) ) {
		echo ' scheme_' . esc_attr( $fcunited_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( fcunited_get_theme_option( 'front_page_features_paddings' ) );
?>"
		<?php
		$fcunited_css      = '';
		$fcunited_bg_image = fcunited_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$fcunited_anchor_icon = fcunited_get_theme_option( 'front_page_features_anchor_icon' );
	$fcunited_anchor_text = fcunited_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( fcunited_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$fcunited_css      = '';
			$fcunited_bg_mask  = fcunited_get_theme_option( 'front_page_features_bg_mask' );
			$fcunited_bg_color_type = fcunited_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $fcunited_bg_color_type ) {
				$fcunited_bg_color = fcunited_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$fcunited_caption = fcunited_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $fcunited_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $fcunited_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $fcunited_caption, 'fcunited_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$fcunited_description = fcunited_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $fcunited_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $fcunited_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $fcunited_description ), 'fcunited_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! fcunited_exists_trx_addons() ) {
					...
					fcunited_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
