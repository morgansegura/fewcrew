	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="fcunited_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.fcunited_post_editor' );
					if ( link.length > 0 ) {
						fcunited_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.fcunited_post_editor', function(e) {
					fcunited_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('fcunited_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function fcunited_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
