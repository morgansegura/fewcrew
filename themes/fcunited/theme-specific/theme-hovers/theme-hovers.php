...
if ( ! function_exists( 'fcunited_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'fcunited_hovers_theme_setup3', 3 );
	function fcunited_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		fcunited_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'fcunited' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'fcunited' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'fcunited' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'fcunited' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'fcunited' ) ),
					'std'      => 'simple',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'fcunited' ),
					),
					'options'  => fcunited_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'fcunited_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'fcunited_hovers_theme_setup9', 9 );
	function fcunited_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'fcunited_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'fcunited_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'fcunited_hovers_responsive_styles', 2000 );
		add_filter( 'fcunited_filter_localize_script', 'fcunited_hovers_localize_script' );
		add_filter( 'fcunited_filter_merge_scripts', 'fcunited_hovers_merge_scripts' );
		add_filter( 'fcunited_filter_merge_styles', 'fcunited_hovers_merge_styles' );
		add_filter( 'fcunited_filter_merge_styles_responsive', 'fcunited_hovers_merge_styles_responsive' );
		add_filter( 'fcunited_filter_get_css', 'fcunited_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'fcunited_hovers_frontend_scripts' ) ) {
	
	function fcunited_hovers_frontend_scripts() {
		if ( fcunited_is_on( fcunited_get_theme_option( 'debug_mode' ) ) ) {
			$fcunited_url = fcunited_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $fcunited_url ) {
				wp_enqueue_script( 'fcunited-hovers', $fcunited_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'fcunited_hovers_frontend_styles' ) ) {
	
	function fcunited_hovers_frontend_styles() {
		if ( fcunited_is_on( fcunited_get_theme_option( 'debug_mode' ) ) ) {
			$fcunited_url = fcunited_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $fcunited_url ) {
				wp_enqueue_style( 'fcunited-hovers', $fcunited_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'fcunited_hovers_responsive_styles' ) ) {
	
	function fcunited_hovers_responsive_styles() {
		if ( fcunited_is_on( fcunited_get_theme_option( 'debug_mode' ) ) ) {
			$fcunited_url = fcunited_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $fcunited_url ) {
				wp_enqueue_style( 'fcunited-hovers-responsive', $fcunited_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'fcunited_hovers_merge_styles' ) ) {
	
	function fcunited_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'fcunited_hovers_merge_styles_responsive' ) ) {
	
	function fcunited_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'fcunited_hovers_localize_script' ) ) {
	
	function fcunited_hovers_localize_script( $arr ) {
		$arr['button_hover'] = fcunited_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'fcunited_hovers_merge_scripts' ) ) {
	
	function fcunited_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'fcunited_hovers_add_icons' ) ) {
	function fcunited_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php fcunited_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'fcunited' ) : esc_html__( 'Buy now', 'fcunited' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'fcunited' );
					} else {
						esc_html_e( 'Details', 'fcunited' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php fcunited_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php fcunited_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php fcunited_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php fcunited_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'fcunited_hovers_get_css' ) ) {
	
	function fcunited_hovers_get_css( $css, $args ) {
