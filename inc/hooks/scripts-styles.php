...
} // end Few_Crew_move_jquery_into_footer
...
function enqueue_theme_scripts() {
  ...
  wp_localize_script( 'scripts', 'Few_Crew_screenReaderText', [
  ...
  wp_localize_script( 'scripts', 'Few_Crew_externalLinkDomains', THEME_SETTINGS['external_link_domains_exclude'] );
} // end Few_Crew_scripts
...
function enqueue_polyfills() {
  ...
  if ( file_exists( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ) ) {
    wp_register_script( 'Few_Crew_legacy', '', [], filemtime( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ), false );
    wp_enqueue_script( 'Few_Crew_legacy' );
    wp_add_inline_script( 'Few_Crew_legacy', $script, true );
  }
