            ...
            foreach( $controls as $k => $v ) {
                if ( ! empty( $v['condition'] ) && is_array( $v['condition'] ) ) {
                    ...
                    foreach( $v['condition'] as $k1 => $v1 ) {
                        if ( substr( $k1, 0, 1 ) == '#' ) {
                            $condition[ str_replace( array( '#page_template', '#', ' ' ), array( 'template', '', '_' ), $k1 ) ] = $v1;
                            $chg = true;
                        } else if ( strpos( $k1, '.editor-page-attributes__template' ) !== false ) {
                            $condition['template'] = $v1;
                            $chg = true;
                        } else if ( substr( $k1, 0, 1 ) == '.' || strpos( $k1, ' ' ) !== false ) {
                            $chg = true;
                        } else {
                            $condition[ $k1 ] = $v1;
                        }
