		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				...
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					if ('joomsport_playerlist' == $table) {
						$existing_columns = $wpdb->get_results( $wpdb->prepare("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'%s' ", $wpdb->prefix . $table), ARRAY_N);
						foreach($existing_columns as $col_id => $col_name){
							$existing_columns[$col_id] = $col_name[0];
						}
						$imported_columns = array_keys ( $rows[0] );
						$diff = array_diff($imported_columns, $existing_columns);

						if (count($diff) > 0) {
							foreach ($diff as $col_n) {
								$wpdb->query( "ALTER TABLE `" . esc_sql( $wpdb->prefix . $table) . "` ADD `" . esc_sql($col_n) . "` FLOAT NOT NULL DEFAULT 0" );
								 file_put_contents('test.txt', 'ALTER = ' . var_export($wpdb->last_error, true) . PHP_EOL, FILE_APPEND);
							}
			...
			if ($this->options['debug'])
                dfl(sprintf(__('Unable to unserialize data from the file %s', 'trx_addons'), trx_addons_get_file_name($this->options['files'][$this->options['demo_type']]['domain_demo']['file_with_' . $slug])));
		}
		$this->response['result'] = $result;
	}
