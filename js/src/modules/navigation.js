...
//           // jscs:enable
//           screenReaderSpan.text(Few_Crew_screenReaderText.expand)
...
//       screenReaderSpanPrev.text(Few_Crew_screenReaderText.expand)
...
//     $(this).attr(
//       'aria-label',
//       `${Few_Crew_screenReaderText.expand_for} ${$(this).prev().text()}`
//     )
//   })
...
//         `${Few_Crew_screenReaderText.collapse_for} ${$(this).prev().text()}`
//         ? `${Few_Crew_screenReaderText.expand_for} ${$(this).prev().text()}`
//         : `${Few_Crew_screenReaderText.collapse_for} ${$(this).prev().text()}`
//     )
//   })
...
//       $(this).attr(
//         'aria-label',
//         $(this).attr('aria-label') === Few_Crew_screenReaderText.expand_toggle
//           ? Few_Crew_screenReaderText.collapse_toggle
//           : Few_Crew_screenReaderText.expand_toggle
//       )
...
//     menu.setAttribute('aria-expanded', 'false')
//     $('#nav-toggle-label').text(Few_Crew_screenReaderText.expand_toggle)
...
const displayBackdrop = (isOpen = false, className = '') => {
  if (!isOpen) {
    $(`.backdrop.${className}`).remove(``)
  } else {
...
const mobileMenuToggle = () => {
  const $navControl = $('.header-nav-control')

  const openMenu = () => {
    $navControl.removeClass('animate-left').addClass('animate-right')
    displayBackdrop(true, 'mobile')
  }
  const closeMenu = () => {
    $navControl.removeClass('animate-right').addClass('animate-left')
    displayBackdrop(false, 'mobile')
  }

  $('.pushy-panel__back-btn-block').on('click', () => {
    closeMenu()
  })

  $('.pushy-panel__toggle.mobile').on('click', () => {
    openMenu()
  })
  ...
  $(window).resize(() => {
    ...
    if ($navControl.hasClass('animate-right') && $vWidth > 1023) {
      $navControl.removeClass('animate-right')
      displayBackdrop(false, 'mobile')
    }
  })
}
